...
export class DataService {
  ...
  private async qurryUserById(id: any): Promise<User | null> {
    try {
      const user = await this.entityManager.findOne(User, {
        where: { id: 1 }
      });
      if (user && user.boss_cookie) {
  ...
  private async updateUser(page: Page, user: User): Promise<void> {
    ...
    if (user == null) {
      ...
      newUser.sex = "男";
      newUser.boss_cookie = "";
      console.log('保存后的用户:', newUser); // 添加日志查看保存后的用户
      const savedUser = await this.entityManager.save(User, newUser);
      console.log('保存后的用户:', savedUser); // 添加日志查看保存后的用户
    } else {
      user.username = "admin";
      user.password = "admin";
